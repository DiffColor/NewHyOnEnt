...
namespace TurtleTools
{
    class AuthTools
    {
        ...
        {
            string normalizedMac = NormalizeMacAddress(macStr);
            if (string.IsNullOrWhiteSpace(normalizedMac) || normalizedMac.Length < 4)
            {
                throw new ArgumentException("유효하지 않은 MAC 주소입니다.", nameof(macStr));
            }
             ...
             * 예) 605718911F5A -> 1F5A
             */
            char[] chArr = normalizedMac.Substring(normalizedMac.Length - 4, 4).ToCharArray();
        ...
        public static bool CheckExistKey(string fpath, string authkey)
        {
            bool _exist = false;

            if (File.Exists(fpath))
            {
                foreach (string strkey in File.ReadAllLines(fpath))
                {
                    if (authkey.Equals(strkey, StringComparison.CurrentCultureIgnoreCase))
                    {
                        _exist = true;
                        break;
                    }
