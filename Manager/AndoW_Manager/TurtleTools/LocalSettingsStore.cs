...
namespace TurtleTools
{
    internal static class LocalSettingsStore
    {
        ...
        public static void EnsureSeeded()
        {
            ...
            lock (SyncRoot)
            {
                ...
                bool firstRun = !File.Exists(FNDTools.GetLocalDbPath());
                LocalConnectionSettings seededConnection = null;
                ...
                using (var db = LocalDbContext.OpenDatabase())
                {
                    ...
                    {
                        seededConnection = BuildConnectionSeed();
                        connCol.Upsert(seededConnection);
                    }

                    if (ftpCol.FindById(ConnectionId) == null)
                    {
                        var seed = BuildFtpSeed();
                        ftpCol.Upsert(seed);
                    }

                    if (uiCol.FindById(ConnectionId) == null)
                    {
                        var seed = BuildUiSeed();
                        uiCol.Upsert(seed);
                    }
                }
                ...
                if (firstRun)
                {
                    TryMigrateServerSettingsFromRethink(seededConnection);
                }
        ...
        private static LocalUiSettings BuildUiSeed()
        {
            ...
            return new LocalUiSettings
            {
                ...
                DefaultWelcomeFontFamily = serverSettings.DefaultWelcomeFontFamily,
                DefaultWelcomeFontSize = serverSettings.DefaultWelcomeFontSize,
                DefaultWelcomeFontColor = serverSettings.DefaultWelcomeFontColor,
                DefaultWelcomeBackgroundColor = serverSettings.DefaultWelcomeBackgroundColor,
                DefaultWelcomeFontColorIndex = serverSettings.DefaultWelcomeFontColorIndex,
                DefaultWelcomeBackgroundColorIndex = serverSettings.DefaultWelcomeBackgroundColorIndex,
                DefaultResolutionOrientation = serverSettings.DefaultResolutionOrientation,
                DefaultResolutionRows = serverSettings.DefaultResolutionRows,
                DefaultResolutionColumns = serverSettings.DefaultResolutionColumns,
                DefaultResolutionWidthPixels = serverSettings.DefaultResolutionWidthPixels,
                DefaultResolutionHeightPixels = serverSettings.DefaultResolutionHeightPixels
            };
        }
        ...
        private static void TryMigrateServerSettingsFromRethink(LocalConnectionSettings connection)
        {
            if (connection == null || string.IsNullOrWhiteSpace(connection.RethinkHost))
            {
                return;
            }
            ...
            try
            {
                int port = connection.RethinkPort > 0 ? connection.RethinkPort : 28015;
                string user = string.IsNullOrWhiteSpace(connection.RethinkUser) ? "admin" : connection.RethinkUser;
                string password = string.IsNullOrWhiteSpace(connection.RethinkPassword) ? "turtle04!9" : connection.RethinkPassword;
                string database = string.IsNullOrWhiteSpace(connection.RethinkDatabase) ? "NewHyOn" : connection.RethinkDatabase;

                RethinkDbContext.Configure(connection.RethinkHost, port, user, password);
                if (!RethinkDbContext.TryConnect(false))
                {
                    return;
                }

                var table = RethinkDbContext.Table(database, "ServerSettings");
                var serverSettings = RethinkDbContext.RunSingleOrDefault<ServerSettings>(table.Get(0));
                if (serverSettings == null)
                {
                    serverSettings = RethinkDbContext.RunList<ServerSettings>(table).FirstOrDefault();
                }
                ...
                SaveFtpSettings(BuildFtpSeed(serverSettings, connection));
                SaveUiSettings(BuildUiSeed(serverSettings));
            }
            catch (Exception ex)
            {
                Logger.WriteErrorLog($"RethinkDB migration failed: {ex}", Logger.GetLogFileName());
            }
        }

        private static LocalFtpSettings BuildFtpSeed(ServerSettings serverSettings, LocalConnectionSettings connection)
        {
            var fallback = BuildFtpSeed();
            if (serverSettings == null)
            {
                return fallback;
            }
            ...
            {
                Id = ConnectionId,
                Host = string.IsNullOrWhiteSpace(serverSettings.DataServerIp)
                    ? (connection?.RethinkHost ?? fallback.Host)
                    : serverSettings.DataServerIp.Trim(),
                Port = serverSettings.FTP_Port > 0 ? serverSettings.FTP_Port : fallback.Port,
                PasvMinPort = serverSettings.FTP_PasvMinPort > 0 ? serverSettings.FTP_PasvMinPort : fallback.PasvMinPort,
        ...
        private static LocalUiSettings BuildUiSeed(ServerSettings serverSettings)
        {
            if (serverSettings == null)
            {
                return BuildUiSeed();
    ...
    internal sealed class LocalUiSettings
    {
        ...
        public string DefaultWelcomeFontFamily { get; set; }
        public double DefaultWelcomeFontSize { get; set; }
        public string DefaultWelcomeFontColor { get; set; }
        public string DefaultWelcomeBackgroundColor { get; set; }
        public int DefaultWelcomeFontColorIndex { get; set; }
        public int DefaultWelcomeBackgroundColorIndex { get; set; }
        public DeviceOrientation DefaultResolutionOrientation { get; set; }
