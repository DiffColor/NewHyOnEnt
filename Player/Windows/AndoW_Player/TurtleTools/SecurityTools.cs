...
namespace TurtleTools
{
    public class SecurityTools
    {
        ...
        #region Firewall
        public static List<string> CreateNetshTCPCmdList(string procName, string procPath , Dictionary<string, int> ports)
        {
            // initialize and open ports command
            List<string> cmdList = CreateOpenTCPPortNetshCmdList(ports);
        ...
        public static List<string> CreateNetshUDPCmdList(string procName, string procPath, Dictionary<string, int> ports)
        {
            // initialize and open ports command
            List<string> cmdList = CreateOpenUDPPortNetshCmdList(ports);
        ...
        public static List<string> CreateOpenTCPPortNetshCmdList(Dictionary<string, int> OPEN_PORTS)
        {
            List<string> cmdList = new List<string>();
        ...
        public static List<string> CreateOpenUDPPortNetshCmdList(Dictionary<string, int> OPEN_PORTS)
        {
            List<string> cmdList = new List<string>();

            string FirewallCmd = "netsh firewall add portopening UDP {1} \"{0}\"";
            string AdvanceFirewallCmd = "netsh advfirewall firewall add rule name=\"{0}\" dir=in action=allow protocol=UDP localport={1}";
        ...
        public static bool NeedToAddRule(string ruleName, int port = -1, bool isTCP = true)
        {
            ...
            try
            {
                using (Process proc = new Process())
                {
                    ...
                    if (System.Environment.OSVersion.Version.Major < VistaMajorVersion)
                    {
                        ...
                        {
                            if (isTCP)
                                DeleteTCPPorts(ruleName, port);
                            else
                                DeleteUDPPorts(ruleName, port);
                        }
                        return true;
                    }
        ...
        public static void DeleteTCPPorts(string ruleName, int port)
        {
            ProcessStartInfo info = null;
            try
            {
        ...
        public static void DeleteUDPPorts(string ruleName, int port)
        {
            ProcessStartInfo info = null;
            try
            {
                using (Process proc = new Process())
                {
                    string FirewallCmd = string.Format("firewall delete portopening protocol=UDP port=\"{0}\"", port);
                    string AdvanceFirewallCmd = string.Format("advfirewall firewall delete rule name=\"{0}\" protocol=UDP localport=\"{1}\"", ruleName, port);
        ...
        public static void GloballyOpenTCPPort(Dictionary<string, int> portDics)
        {
            INetFwMgr mgr = (INetFwMgr)Activator.CreateInstance(NetFwMgrType);
        ...
        public static void GloballyOpenUDPPort(Dictionary<string, int> portDics)
        {
            INetFwMgr mgr = (INetFwMgr)Activator.CreateInstance(NetFwMgrType);
